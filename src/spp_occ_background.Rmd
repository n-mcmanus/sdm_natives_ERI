...
#### FMMP
Newer years (2016-present) put data in .gdb format rather than providing all the .shp files directly. Use the following fxn to extract all the layers, isolate prime farmland, and save as one .shp file for the year
```{r}
## Read in all layers (counties) of .gdb
path <- here("data/fmmp/2018/Important_Farmland_2018.gdb")
lyrs <- st_layers(path)
fxn <- function(lyr=lyrs$name) {
  x <- st_read(dsn = path, layer=lyr) %>% 
    dplyr::filter(polygon_ty == "P")
}
prime_farms <- purrr::map(lyrs$name, fxn, .progress=TRUE)
...
## Save .shp
filename <- here("data/fmmp/2018/prime_farmland_2018.shp")
write_sf(counties, filename)

```

For 2000-2014, we'll do the same process, but the code slightly differs because they aren't packaged in a .gdb
```{r}
...
```{r}
shp_list_df <- data.frame(shp = list.files(here("data/fmmp/prime_farmland/"), 
...
for(p in 1:length(names)){
  ...
  plant_ext <- data.frame()
    ...
    for(i in 1:nrow(shp_list_df)){
      shp <- vect(shp_list_df$shp[i])
      
      if(i != nrow(shp_list_df)) {
        x_range <- x %>% 
               filter(year >= shp_list_df$year[i] 
                      & year < shp_list_df$year[i+1])
      ##If filtering to last date, no next row to look at
      ##Just do 2021-present
      } else {
        x_range <- x %>% 
