...
## Years of interest
```{r}
## filter to 2000-2022 water years
num <- sppOcc %>% 
  select(gbifid, species, decimallatitude, decimallongitude, month, year) %>% 
...
```{r}
## Testing difference between fxns in two packages
library(spThin)
library(enmSdmX)
...
enmTest <- elimCellDuplicates(sppOcc, aet2020, longLat = c("decimallongitude", "decimallatitude")) %>% 
...
```{r}
## Using enmSdmX ------------------
samp <- sampleRast(aet2020, 10560, prob = FALSE, replace = FALSE)

samp <- as.data.frame(samp)

samp_vect <- vect(samp, geom = c("x", "y"), crs = "WGS84")
plot(samp_vect)


## Using dismo ------------------------
### convert to raster obj
aet2020_r <- raster(aet2020)

### random samples w/o replacement
sampOcc <- randomPoints(aet2020_r, 11040, prob = FALSE)
...
dates <- data.frame(year = rep(2000:2022, each = 480), 
                    month = rep(1:12, each = 40))
dates <- rbind(tail(dates, 120), head(dates, -120))
dates[1:120, 1] = (dates[121,1] - 1)
