...
natsgo_agg(horizon, component, mapunit, 
...
soil_df <- read_csv(here("data/natsgo/horizon_200cm_CA.csv")) %>% 
  ## turn taxon into a factor, then reclass as numeric
  ## must be in numeric form to reclassify raster
  mutate(taxon = as.factor(compname)) %>% 
  mutate(taxon = unclass(taxon)) %>%
...
soil_vars = data.frame(var = c("cec", "ph", "om", "taxon"),
