...
Assessing the correlation between environmental variables for SDM. Then generating a Pearson correlation matrix.

```{r}
vars1 <- swd %>% 
  filter(presence == 1) %>% 
  dplyr::select(aet:tdiff)
vars2 <- occExtract_2000_2022 %>% 
  filter(!is.na(aet)) %>% 
  dplyr::select(tmn, tmx)
