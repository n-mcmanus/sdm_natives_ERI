...
## Download data (Survey area CA666 in Kern Co)
ssurgo666 <- get_ssurgo(template = "CA666", label = "CA_666") #add put raw.dir later if this works

## create chorizon, component, 
## and mapunit dataset for aoi
chorizon <- ssurgo666$tabular$chorizon
component <- ssurgo666$tabular$component
mapunit <- ssurgo666$tabular$mapunit
...
# [1] 242
### Component data for all map units
...
## weighted average of variables based on % component in mapunit
...
full_soil <- full_soil %>% 
  ...
  dplyr::select(!c(mukind)) %>% 
  ## convert mukey from dbl to char for spatial join step
  mutate(mukey = as.character(mukey))


## Map unit merge --------------------------------------------------------------
## Now we'll merge it with the spatial data
ssurgo_sf <- ssurgo666$spatial %>% 
  janitor::clean_names() %>% 
